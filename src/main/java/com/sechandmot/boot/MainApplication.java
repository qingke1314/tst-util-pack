 ...
 * @SpringBootConfiguration 表示这是一个配置类，会被 Spring 扫描到
 * @EnableAutoConfiguration 表示开启自动配置，会根据项目的依赖自动配置 Spring 容器
 *    AutoConfigurationPackage 利用Register类给容器导入一系列组件 将main类所在包下的组件都导入进来
 *    AutoConfigurationImportSelector 给容器导入127个自动配置组件(利用Conditional注解按需生效)
